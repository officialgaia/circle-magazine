...
- 認証: **Firebase Authentication**(Googleサインインのみ)
- データベース: **Cloud Firestore**
- ファイル保管: **Firebase Storage**

利用者はサークル内のメンバーに限定されます(ログインしないと中身は見えません)。
...
管理者判定は画面上の出し分けだけでなく、`firestore.rules` / `storage.rules` によってサーバー側でも強制しています。
...
- `issues/{issueId}` — 号(`year` / `title` / `status`)
  - `submissions/{submissionId}` — メンバーがアップロードした生ファイル
  - `booklet/{sectionId}` — 管理者が構成した、完成した冊子のセクション
  - `roster/{rosterId}` — 名簿(`name` / `email` / `submitted` / `note`)
...
2. **Authentication** で「Google」サインイン方法を有効化する。
3. **Firestore Database** と **Storage** を本番モードで作成する(サークル規模なら無料枠で収まります)。
...
管理者としてログイン後、`/admin` から新年度号を作成し、各号の `/issues/[issueId]/admin` の名簿欄からメンバー(名前・Googleアカウント)を登録してください。
